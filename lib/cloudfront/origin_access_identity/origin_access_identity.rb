...
class Cloudfront
  module OriginAccessIdentity
    ...
    ORIGIN_ACCESS_IDENTITY_URL = "/#{Cloudfront::API_VERSION}/origin-access-identity/cloudfront"

    def origin_access_identity_create(origin_access_identity = nil)
      origin_access_identity ||= OriginAccessIdentity.new
      connection.post do |request|
        request.url ORIGIN_ACCESS_IDENTITY_URL
      ...
      def origin_access_identity_get(origin_access_identity_id)
        connection.get ORIGIN_ACCESS_IDENTITY_URL + '/' + origin_access_identity_id
      end

      def origin_access_identity_get_config(origin_access_identity_id)
        connection.get ORIGIN_ACCESS_IDENTITY_URL + '/' + origin_access_identity_id + '/config'
      end

      def origin_access_identity_put_config(origin_access_identity_id, origin_access_identity, etag)
        connection.put do |request|
          request.url ORIGIN_ACCESS_IDENTITY_URL + '/' + origin_access_identity_id + '/config'
      ...
      def origin_access_identity_delete(origin_access_identity_id)
        ...
        connection.delete do |request|
          request.url ORIGIN_ACCESS_IDENTITY_URL + '/' + origin_access_identity_id
          request.headers['If-Match'] = etag
        end
      end
